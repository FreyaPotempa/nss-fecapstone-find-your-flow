...
import { Button, Card, CardBody, Container, Heading, Image, Input, SimpleGrid, Stack, Text, Tooltip, useColorMode } from "@chakra-ui/react";
...
export const FlowDetail = () => {
    ...
    <Container key={`flow__${flow.id}`} >
        ...
        <Text fontWeight='bold'>Difficulty: {flow?.difficulty}/5</Text>
            <SimpleGrid border="1px dashed lightgrey" m="12px" p="8px" width="100%" spacing={4} templateColumns='repeat(auto-fill, minmax(150px, 1fr))'>
        {
            flow?.poseColumnIdList?.map((poseId) => {
                ...
                return <Card maxW='175px' align="center" key={`pose__${poseObj?.id}`}>
                    <CardBody>
                <Image src={poseObj?.img_url} sx={ colorMode === "light" ? "" : { filter: 'invert(1)'} 
                } 
           ...
           <>
            ...
            {
                favesByUser.find((fave) => fave.flowId === flow.id) ? <Tooltip label='Remove from favorites'><Button m="6px" type="button" onClick={deleteFaveFlow}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" height={20}>
                <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clipRule="evenodd" />
              </svg>
                </Button></Tooltip> :
            
            <Tooltip label='Save this Flow'><Button m="6px"  type="button" onClick={favoriteFlow}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" height={20}>
